...
PressureSolverParallel::PressureSolverParallel(std::shared_ptr<Discretization> discretization, double epsilon, int maximumNumberOfIterations, Partitioning partitioning) : PressureSolver(discretization, epsilon, maximumNumberOfIterations), partitioning_(partitioning),
...
void PressureSolverParallel::receiveAndSendPressuresFromAndToOtherProcesses(bool secondHalfStep)
{
    ...
    if (!partitioning_.ownPartitionContainsTopBoundary())
    {
        ...
        MPI_Wait(&receiveUpperRequest, &recvStatus);
        // if (recvStatus.MPI_ERROR != MPI_SUCCESS)
        //     std::cout << "R: " << recvStatus.MPI_ERROR << std::endl;
        ...
        for (int i = pIBegin_ + !upperOffset_[secondHalfStep]; i < pIEnd_; i += 2)
        {
            ...
            if (std::isnan(receiveUpperPBuffer[k]))
            {
                std::cout << partitioning_.ownRankNo() << " receives Nan" << std::endl;
            }
        }
    }

    MPI_Status sendStatus[sendRequests.size()];
    MPI_Waitall(sendRequests.size(), sendRequests.data(), sendStatus);

    // for (int v = 0; v < sendRequests.size(); v++)
    // {
    //     if (sendStatus[v].MPI_ERROR != MPI_SUCCESS)
    //         std::cout << "S: " << sendStatus[v].MPI_ERROR << std::endl;
    // }
}
